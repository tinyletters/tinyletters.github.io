...
const countWords = (text) => {
    ...
    words.forEach((word) => {
        ...
        if (!stopWords.has(normalizedWord)) { // Exclude stop words
...
const getBubbleData = (frequency) => {
    ...
    data.forEach(entry => {
        const words = entry.birthStory.toLowerCase()
            .replace(/[.,!?;]*/g, '')
            .split(/\s+/);
        
        words.forEach(word => {
            if (frequency[word] >= 2) { // Only include words with a frequency greater than 2
...
const BubbleChartComponent = () => {
    ...
    useEffect(() => {
        ...
        const filtered = data.filter(entry => {
            return (
                (race === '' || entry.race === race) &&
                (country === '' || entry.country === country) &&
                (birthType === '' || entry.birthKind === birthType)
            );
        });
    ...
    useEffect(() => {
        ...
        const simulation = d3.forceSimulation(bubbleData)
            ...
            .force('collide', d3.forceCollide(d => d.size + 20));
        ...
        const bubbles = svg.selectAll('circle')
            ...
            .on('mouseover', (event, d) => {
                ...
                const boldedSentence = relevantSentence.replace(
                    new RegExp(`(${d.name}|${normalizeWord(d.name)})`, 'gi'), 
                    '<strong>$1</strong>'
                );
